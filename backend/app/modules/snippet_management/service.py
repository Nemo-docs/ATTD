...
class SnippetManagementService:
    ...
    def __init__(self):
        ...
        self.snippets: AsyncCollection = self.db[self.snippets_collection_name]
        self.collections: AsyncCollection = self.db[self.collections_collection_name]
    ...
    async def _ensure_indexes(self) -> None:
        ...
        if not self._indexes_created:
            try:
                await self.snippets.create_index("id", unique=True)
                await self.snippets.create_index([("user_id", 1), ("created_at", -1)])
                await self.collections.create_index("user_id", unique=True)
                self.logger.info("Created indexes for snippets module")
                self._indexes_created = True
            except PyMongoError as e:
                self.logger.error(f"Could not create index: {e}")

    async def create_snippet(self, user_id: str, content: str, tags: Optional[List[str]] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            saved = await self._save_snippet(snippet_data)
            if not saved:
                return {"error": "Failed to save snippet"}

            # Add the id to user's snippets collection (upsert)
            await self._add_snippet_to_collection(user_id, snippet_model.id)
    ...
    async def get_snippet(self, snippet_id: str, user_id: Optional[str] = None) -> Optional[Dict[str, Any]]:
        try:
            await self._ensure_indexes()
            if not snippet_id:
                return None
            return await self._get_snippet(snippet_id, user_id=user_id)
        except Exception as e:
            self.logger.error(f"Error retrieving snippet: {e}")
            return {"error": str(e)}

    async def get_all_snippets(self, user_id: Optional[str] = None) -> Dict[str, Any]:
        try:
            await self._ensure_indexes()
            if not user_id:
                return {"error": "user_id is required"}

            snippets_data = await self._get_all_snippets(user_id=user_id)
            snippets = []
    ...
    async def update_snippet(self, snippet_id: str, user_id: str, content: Optional[str] = None, tags: Optional[List[str]] = None, add_tags: Optional[List[str]] = None) -> Dict[str, Any]:
        try:
            await self._ensure_indexes()
            if not user_id:
                return {"error": "user_id is required"}

            existing = await self._get_snippet(snippet_id, user_id=user_id)
            if not existing:
            ...
            success = await self._update_snippet(snippet_id, user_id, update_data)
            if not success:
                return {"error": "Failed to update snippet"}

            updated = await self._get_snippet(snippet_id, user_id=user_id)
            if updated:
    ...
    async def delete_snippet(self, snippet_id: str, user_id: str) -> Dict[str, Any]:
        try:
            await self._ensure_indexes()
            if not user_id:
                return {"error": "user_id is required"}
            ...
            success = await self._delete_snippet(snippet_id, user_id)
            if not success:
                return {"error": "Failed to delete snippet"}

            # remove from collection
            await self._remove_snippet_from_collection(user_id, snippet_id)
            success = await self._add_deleted_snippet_to_collection(user_id, snippet_id)
            if not success:
    ...
    async def sync_snippets(self, user_id: str, local_snippets: List[Dict[str, Any]], client_last_edit_unix: Optional[int] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            server_last_unix = await self._get_collection_last_unix(user_id)
            current_snippets = await self._get_all_snippets(user_id=user_id)
            existing_ids: Set[str] = {snippet["id"] for snippet in current_snippets}

            # remove deleted snippets from local snippets
            deleted_snippet_ids = await self._get_deleted_snippet_ids(user_id)
            local_snippets = [snippet for snippet in local_snippets if snippet.get("id") not in deleted_snippet_ids]
            created_ids = await self._merge_local_snapshots(user_id, local_snippets, existing_ids)
            merged_snippets = await self._get_all_snippets(user_id=user_id)
            ...
            await self._set_collection_last_unix(user_id, last_unix_timestamp)
    ...
    async def get_sync_status(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            await self.ensure_collection(user_id)
            last_unix_timestamp = await self._get_collection_last_unix(user_id)
            query = {"$or": [{"user_id": user_id}, {"user_id": {"$exists": False}}]}
            total_count = await self.snippets.count_documents(query)
    ...
    async def _save_snippet(self, snippet_data: Dict[str, Any]) -> bool:
        try:
            now = self._current_iso_timestamp()
            snippet_data["created_at"] = now
            snippet_data["updated_at"] = now
            result = await self.snippets.replace_one({"id": snippet_data["id"]}, snippet_data, upsert=True)
    ...
    async def _get_snippet(self, snippet_id: str, user_id: Optional[str] = None) -> Optional[Dict[str, Any]]:
        try:
            query = {"id": snippet_id}
            if user_id:
                query["user_id"] = user_id

            result = await self.snippets.find_one(query)
            if not result and user_id:
                fallback = await self.snippets.find_one({"id": snippet_id, "user_id": {"$exists": False}})
                if fallback:
                    await self.snippets.update_one({"_id": fallback["_id"]}, {"$set": {"user_id": user_id}})
    ...
    async def _get_all_snippets(self, user_id: Optional[str] = None) -> List[Dict[str, Any]]:
        try:
            query = {}
            if user_id:
                query = {"$or": [{"user_id": user_id}, {"user_id": {"$exists": False}}]}
    ...
    async def _update_snippet(self, snippet_id: str, user_id: str, update_data: Dict[str, Any]) -> bool:
        try:
            result = await self.snippets.update_one({"id": snippet_id, "user_id": user_id}, {"$set": update_data})
            if result.modified_count == 0 and user_id:
                result = await self.snippets.update_one({"id": snippet_id, "user_id": {"$exists": False}}, {"$set": {**update_data, "user_id": user_id}})
            return result.modified_count > 0
        except PyMongoError as e:
            self.logger.error(f"Error updating snippet: {e}")
            raise e

    async def _delete_snippet(self, snippet_id: str, user_id: str) -> bool:
        try:
            result = await self.snippets.delete_one({"id": snippet_id, "user_id": user_id})
            if result.deleted_count == 0 and user_id:
                result = await self.snippets.delete_one({"id": snippet_id, "user_id": {"$exists": False}})
            return result.deleted_count > 0
        except PyMongoError as e:
            self.logger.error(f"Error deleting snippet: {e}")
            raise e

    async def _get_deleted_snippet_ids(self, user_id: str) -> List[str]:
        try:
            result = await self.collections.find_one({"user_id": user_id}, {"deleted_snippet_ids": 1})
            return result.get("deleted_snippet_ids", [])
        except PyMongoError as e:
            self.logger.error(f"Error getting deleted snippet ids: {e}")
            raise e

    async def _add_deleted_snippet_to_collection(self, user_id: str, snippet_id: str):
        try:
            await self.collections.update_one({"user_id": user_id}, {"$addToSet": {"deleted_snippet_ids": snippet_id}})
            return True
        except PyMongoError as e:
    ...
    # --- Collection helpers ---
    async def _add_snippet_to_collection(self, user_id: str, snippet_id: str) -> None:
        try:
            now = datetime.utcnow()
            last_unix_timestamp = int(now.timestamp())
    ...
    async def _remove_snippet_from_collection(self, user_id: str, snippet_id: str) -> None:
        try:
            now = datetime.utcnow()
            last_unix_timestamp = int(now.timestamp())
            await self.collections.update_one(
    ...
    async def ensure_collection(self, user_id: str) -> Dict[str, Any]:
        """
        ...
        try:
            if not user_id:
                return {"error": "user_id is required"}
            if await self.collections.find_one({"user_id": user_id}):
                return {"message": "Snippets collection already exists"}
            now = datetime.now(timezone.utc)
            last_unix_timestamp = int(now.timestamp())
            await self.collections.insert_one({
                "user_id": user_id,
                "snippet_ids": [],
                "created_at": now,
                "updated_at": now,
    ...
    # --- Sync helpers ---
    async def _merge_local_snapshots(self, user_id: str, local_snippets: List[Dict[str, Any]], existing_ids: Set[str]) -> List[str]:
        """
        ...
        for snippet in local_snippets:
            ...
            if await self._save_snippet(snippet_model.dict()):
                await self._add_snippet_to_collection(user_id, snippet_model.id)
                existing_ids.add(snippet_model.id)
                created_ids.append(snippet_model.id)
        return created_ids

    async def _get_collection_last_unix(self, user_id: str) -> int:
        """
        Fetch the stored collection unix timestamp for the user.
        """
        doc = await self.collections.find_one({"user_id": user_id}, {"last_unix_timestamp": 1})
        # print("doc from _get_collection_last_unix()", doc)
        if doc and doc.get("last_unix_timestamp"):
            return int(doc["last_unix_timestamp"])
        return 0

    async def _set_collection_last_unix(self, user_id: str, last_unix_timestamp: int) -> None:
        """
