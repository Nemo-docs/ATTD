...
async def create_snippet(request: Request, snippet_request: CreateSnippetRequest) -> CreateSnippetResponse:
    ...
    try:
        ...
        result = await snippet_service.create_snippet(user_id=user_id, content=snippet_request.content, tags=snippet_request.tags)
        if "error" in result:
...
async def get_all_snippets(request: Request) -> GetSnippetsResponse:
    ...
    try:
        ...
        result = await snippet_service.get_all_snippets(user_id=user_id)
        if "error" in result:
...
async def get_sync_status(request: Request) -> SyncStatusResponse:
    ...
    try:
        ...
        result = await snippet_service.get_sync_status(user_id=user_id)
        if "error" in result:
            status_code_value = status.HTTP_400_BAD_REQUEST if result["error"] == "user_id is required" else status.HTTP_500_INTERNAL_SERVER_ERROR
...
async def sync_snippets(request: Request, sync_request: SyncSnippetsRequest) -> SyncSnippetsResponse:
    ...
    try:
        ...
        result = await snippet_service.sync_snippets(
            user_id=user_id,
...
async def ensure_collection(request: Request) -> EnsureCollectionResponse:
    ...
    try:
        ...
        result = await snippet_service.ensure_collection(user_id=user_id)
        if "error" in result:
            status_code_value = status.HTTP_400_BAD_REQUEST if result["error"] == "user_id is required" else status.HTTP_500_INTERNAL_SERVER_ERROR
...
async def get_snippet(request: Request, snippet_id: str) -> GetSnippetResponse:
    ...
    try:
        ...
        result = await snippet_service.get_snippet(snippet_id, user_id=user_id)
        if result is None:
...
async def update_snippet(request: Request, snippet_id: str, update_request: UpdateSnippetRequest) -> UpdateSnippetResponse:
    ...
    try:
        ...
        result = await snippet_service.update_snippet(snippet_id=snippet_id, user_id=user_id, content=update_request.content, tags=update_request.tags, add_tags=update_request.add_tags)
        if "error" in result:
...
async def delete_snippet(request: Request, snippet_id: str) -> DeleteSnippetResponse:
    ...
    try:
        ...
        result = await snippet_service.delete_snippet(snippet_id=snippet_id, user_id=user_id)
        if "error" in result:
            if "not found" in result["error"].lower():
                raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=result["error"])
            else:
